...
class NOAA_OCS(object):
    ...
    def __init__(self,
                 obs=None):
        """
        Class constructor.  May be called directly with an existing
        DataFrame, but typically it will be most useful to use the
        class method parse_file to create a NOAA_OCS object from a
        NOAA airborne obseration CSV
        file.
        PARAMETERS
        ----------
    ...
    def parse_file(cls, fname):
        """
        parses a NOAA airborne measurement whitespace-delimited ASCII
        file to a NOAA_OCS object.  The last comment line in the file
        header should contain the column names in the format
        "data_fields: NAME_1 NAME_2 NAME_3 ... NAME_N".  There are no
        restrictions on the number of variables in the file, but the
        following variables are required: sample_longitude,
        sample_latitude, sample_year, sample_month, sample_day,
        sample_hour, sample_minute, sample_seconds.  The timestamp is
        converted to a datetime.datetime object and placed in the obs
        DataFrame as obs.datet.
        PARAMETERS
        ----------
